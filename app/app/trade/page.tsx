...
function computeMarketCapAtSupply(
    supplyRaw: number,
    basePriceRaw: number,
    stepSizeRaw: number,
...
export default function TradePage() {
    ...
    const [holderAccounts, setHolderAccounts] = useState<
    ...
    const sellAmount = amount && !isNaN(Number(amount))
        ? parseUnits(amount, 8)
        : 0n;
    const buyAmountTinybar = amount && !isNaN(Number(amount))
        ? parseUnits(amount, 8)
        : 0n;
    const buyAmountWeibar = amount && !isNaN(Number(amount))
        ? parseEther(amount)
        : 0n;
    ...
    useEffect(() => {
        ...
        const increment = sSize > 0
            ? Math.max(sSize / 2, 2500 * 1e8)
            : 5000 * 1e8;
        const pastPoints = 48;
        const futurePoints = 20;
        const startSupply = Math.max(
            0,
            liveSupply - increment * (pastPoints - 1),
        );
        ...
        for (let i = 0; i < pastPoints + futurePoints; i++) {
            ...
            const point = computeMarketCapAtSupply(
                supplyPoint,
                bPrice,
                sSize,
                gRate,
    ...
    useEffect(() => {
        ...
        async function fetchHolderCount() {
            try {
                let nextUrl =
                    `${MIRROR_NODE_BASE_URL}/tokens/${TOKEN_ID}/balances?limit=100`;
                let count = 0;
                ...
                while (nextUrl) {
                    ...
                    if (!response.ok) {
                        throw new Error(
                            `Mirror node error: ${response.status}`,
                        );
                    }
                    ...
                    const balances = Array.isArray(data.balances)
                        ? data.balances
                        : [];
                    count += balances.filter(
                        (entry: { balance?: number | string }) =>
                            Number(entry.balance ?? 0) > 0,
                    ).length;
                        ...
                        collected
                            ...
                            .map((holder) => ({
                                ...
                                balance: formatTokenValue(
                                    BigInt(Math.trunc(Number(holder.balance))),
                                ),
                            })),
                    );
                }
    ...
    useEffect(() => {
        ...
        async function fetchVolumes() {
            try {
                let nextUrl =
                    `${MIRROR_NODE_BASE_URL}/contracts/${CONTRACT_ADDRESS}/results/logs?order=asc&limit=100`;
                let totalBuy = 0n;
                let totalSell = 0n;
                    ...
                    for (const log of logs) {
                        try {
                            ...
                            if (decoded.eventName === "Bought") {
                                totalBuy += decoded.args.hbarSpent as bigint;
                            }
                            if (decoded.eventName === "Sold") {
                                totalSell += decoded.args
                                    .hbarReturned as bigint;
                            }
                        } catch {
                            continue;
                        }
    ...
    const handleWithdrawAll = () => {
        if (
            !address || !contractBalance?.value || contractBalance.value <= 0n
        ) {
            return;
        }
        writeContract({
            ...
            args: [
                ...
                weibarToTinybar(contractBalance.value),
            ],
            chainId: CHAIN_ID,
        });
    };
    ...
    const marketCapTinybar = currentPrice !== undefined
        ? (totalSupplyRaw * (currentPrice as bigint)) /
        10n ** BigInt(TOKEN_DECIMALS)
        : undefined;
    const holdingsValueTinybar =
        currentPrice !== undefined && userTokenBalance !== undefined
            ? ((userTokenBalance as bigint) * (currentPrice as bigint)) /
            10n ** BigInt(TOKEN_DECIMALS)
            : undefined;
    const currentPriceHbar = currentPrice !== undefined
        ? formatHbarValue(currentPrice as bigint)
        : "...";
    const projectedFdvHbar = basePrice !== undefined &&
        stepSize !== undefined &&
        growthRateBP !== undefined &&
        ...
        ? formatHbarNumber(
            computeMarketCapAtSupply(
                ...
                Math.max(Number(stepSize) * 10, 50000 * 1e8),
                Number(basePrice),
                Number(stepSize),
                Number(growthRateBP),
            ).marketCapHbar,
        ...
        >
            ...
            >
                    ...
                    >
                        ...
                        >
                            <div>
                                ...
                                >
                                    {formatTokenValue(
                                        currentSupply as bigint | undefined,
                                    )}
                                </div>
                                <div
                                    style={{
                            ...
                            <div>
                                ...
                                >
                                    {holdingsValueTinybar !== undefined
                                        ? formatHbarValue(holdingsValueTinybar)
                                        : "0.00"}
                                </div>
                                <div
                                    style={{
                ...
                >
                    <div
                        className="glass-card"
                        style={{ padding: 24, flex: "1 1 720px", minWidth: 0 }}
                    >
                        <div
                            style={{
                                display: "flex",
                        ...
                        >
                            ...
                            >
                                <div style={{ textAlign: "right" }}>
                                    ...
                                    >
                                        {marketCapTinybar !== undefined
                                            ? formatHbarValue(marketCapTinybar)
                                            : "..."}{" "}
                                        <span
                                            style={{
                                                fontSize: 13,
                        ...
                        >
                            ...
                            >
                                <div>
                                    ...
                                    >
                                        Largest visible wallets from Hedera
                                        Mirror Node
                                    </div>
                                </div>
                                <div
                                ...
                                >
                                    {holderCount === null
                                        ? "Loading..."
                                        : `${holderCount.toLocaleString()} total holders`}
                                </div>
                            </div>
                            ...
                            >
                                {holderAccounts.length > 0
                                    ? (
                                        holderAccounts.map((holder, index) => (
                                            <div
                    ...
                    >
                        ...
                        {Number(amount) > 0 &&
                            (activeTab === "buy" ? previewBuy : previewSell) &&
                            (
                                <div
                                    style={{
                                        padding: 18,
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        >
                                            {activeTab === "buy"
                                                ? `${formatUnits(
                                                    (previewBuy as unknown as bigint[])
                                                    ?.[0] || 0n,
                                                    8,
                                                )
                                                } NANO`
                                                : `${formatUnits(
                                                    (previewSell as unknown as bigint[])
                                                    ?.[0] || 0n,
                                                    8,
                                                )
                                                } HBAR`}
                                ...
                                : (
                                    ...
                                    >
                                            ...
                                            (
                                                ...
                                                >
                                                    {allowance && allowance > 0n
                                                        ? `Increase Approval to ${amount} NANO`
                                                        : `Approve ${amount || "0"
                                                        } NANO for Sell`}
                                                </button>
                                            )}
                                        ...
                                        >
                                            {isPending || isWaiting
                                                ? "PROCESSING..."
                                                : `${activeTab.toUpperCase()} ${activeTab === "buy"
                                                    ? "NANO"
                                                    : "FOR HBAR"
                                                }`}
                                        </button>
